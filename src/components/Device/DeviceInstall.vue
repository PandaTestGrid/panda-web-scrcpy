...
const install = async () => {
  ...
  try {
    ...
    const installResult: unknown = await pm.installStream(
      file.value.size,
      fileStream,
      options
    );
...
const isInstallDisabled = computed(
  () => !client.device || installing.value || !file.value
);
...
<template>
  <v-container
    class="pa-4 d-flex flex-column device-install-container"
    :class="{ 'px-16': $vuetify.display.lgAndUp }"
    style="height: 100%"
  >
    <v-row class="flex-grow-1 overflow-auto">
      <v-col cols="12" class="d-flex flex-column">
        ...
        <v-row class="mb-4">
          <v-col cols="12" sm="6">
            <v-checkbox
              v-model="options.allowTest"
              label="允许测试包"
              density="compact"
              hide-details
            />
            <v-checkbox
              v-model="options.internalStorage"
              label="强制安装到内部存储"
              density="compact"
              hide-details
            />
          ...
          <v-col cols="12" sm="6">
            <v-checkbox
              v-model="options.requestDowngrade"
              label="允许降级安装"
              density="compact"
              hide-details
            />
            <v-checkbox
              v-model="options.grantRuntimePermissions"
              label="授予所有运行时权限"
              density="compact"
              hide-details
            />
